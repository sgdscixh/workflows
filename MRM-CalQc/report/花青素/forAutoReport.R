...
get_table_quant <- function(l.a = l.a, filename = NA, overwrite = TRUE, sheet = "merge") {
  ...
  # 修改：从 Table S5_quantification.xlsx 读取数据
  quant_file <- paste0(l.a$resDir, "/Table S5_quantification.xlsx")

  # 读取下机浓度sheet
  data_q1 <- readxl::read_xlsx(quant_file, sheet = "下机浓度", na = "#N/A")

  # 读取样本浓度sheet
  data_q2 <- readxl::read_xlsx(quant_file, sheet = "样本浓度", na = "#N/A")

  # 获取化合物名称列表
  cpds <- data_q1[[1]] %>% unlist(use.names = FALSE)

  # # 修改列名（添加单位）
  # colnames(data_q1)[1] <- sprintf('%s (%s)', molecule_name, l.a$unit_CC)
  # colnames(data_q2)[1] <- sprintf('%s (%s)', molecule_name, l.a$unit_CM)
  if (length(qc_name_rsd) > 0) {
    data_qc <- qc_name_rsd
  } else {
    data_qc <- NULL
  }
...
# 新增脂肪酸含量计算公式
get_table_quant_modified <- function(l.a = l.a, filename = NA, overwrite = TRUE, sheet = "merge", mw_file_path = NULL) {
  # 如果未提供MW文件路径，则使用默认路径
  if (is.null(mw_file_path)) {
    mw_file_path <- paste0(l.a$resDir, "/Table.xlsx")
  }
  ...
  # 读取MW信息
  mw_data <- openxlsx::read.xlsx(mw_file_path, sheet = "MW", na.strings = "#N/A")

  # 转换样本浓度数据
  data_q2_converted <- convert_concentration_data(data_q2, mw_data, molecule_name, l.a$unit_CM)
  ...
  colnames(data_q1)[1] <- sprintf("%s (%s)", molecule_name, l.a$unit_CC)
  colnames(data_q2_converted)[1] <- sprintf("%s (%s)", molecule_name, l.a$unit_CM)
  ...
  writeDataTable(wb, sheet = 1, data_q1, keepNA = T, na.string = "N/A", withFilter = F)
  addWorksheet(wb, sheetName = "样本浓度")
  writeDataTable(wb, sheet = 2, data_q2_converted, keepNA = T, na.string = "N/A", withFilter = F)
  setColWidths(wb, sheet = 1, cols = 1, "auto")
  setColWidths(wb, sheet = 2, cols = 1, "auto")
  setRowHeights(wb, sheet = 1, rows = 1, 20)
  setRowHeights(wb, sheet = 1, rows = 2:(nrow(data_q1) + 1), 16)
  setRowHeights(wb, sheet = 2, rows = 1, 20)
  setRowHeights(wb, sheet = 2, rows = 2:nrow(data_q2_converted), 16)
  addStyle(
    wb,
    sheet = 1, rows = 1:(nrow(data_q1) + 1), cols = 2:ncol(data_q1),
    gridExpand = T, stack = T, style = createStyle(halign = "center")
  ...
  addStyle(
    wb,
    sheet = 2, rows = 1:(nrow(data_q2_converted) + 1), cols = 2:ncol(data_q2_converted),
    gridExpand = T, stack = T, style = createStyle(halign = "center")
  )
  addStyle(
    wb,
  ...
  addStyle(
    wb,
    sheet = 2, rows = 1:(nrow(data_q2_converted) + 1), cols = 1:ncol(data_q2_converted),
    gridExpand = T, stack = T, style = createStyle(valign = "center")
  )
  saveWorkbook(wb, filename, overwrite = overwrite)
...
# 辅助函数：转换浓度数据
convert_concentration_data <- function(data_q2, mw_data, molecule_name, unit_CM) {
  # 获取化合物名称列
  compound_col <- sprintf("%s (%s)", molecule_name, unit_CM)

  # 确保列名匹配
  if (!compound_col %in% colnames(data_q2)) {
    # 如果标准列名不存在，尝试使用第一列
    compound_col <- colnames(data_q2)[1]
  }

  # 合并数据
  merged_data <- data_q2 %>%
    left_join(mw_data, by = setNames("Compound.Name.(ng/mL)", compound_col))

  # 检查是否成功匹配
  if (all(is.na(merged_data$FAME_MW)) || all(is.na(merged_data$MW))) {
    warning("未能成功匹配MW数据，请检查化合物名称是否一致")
    return(data_q2)
  }

  # 获取数值列（排除化合物名称列和MW相关列）
  numeric_cols <- setdiff(colnames(data_q2), compound_col)

  # 应用转换公式：新的数据 = 原始数据 / FAME_MW * MW
  for (col in numeric_cols) {
    merged_data[[col]] <- as.numeric(merged_data[[col]]) / merged_data$FAME_MW * merged_data$MW
  }
...
get_table_calib <- function(l.a = l.a, filename = NA, sheet = "merge", sheetName = "Calibration", overwrite = TRUE) {
  ...
  data_cal <- get_table_sheet(paste0(l.a$resDir, "/Table S3-Calibration.xlsx"), sheet = sheet)
...
get_table_stats <- function(l.a = l.a, filename = NA, fill = NA, sheet = "stats") {
  data_rsd <-
    paste0(l.a$resDir, "/Table S4-Accuracy and RSD.xlsx") %>%
    get_table_sheet(sheet = sheet)

  # 使用更安全的列访问方式
  if ("Accuracy[%]" %in% colnames(data_rsd)) {
    data_rsd[["Accuracy[%]"]] <- data_rsd[["Accuracy[%]"]] %>% round(1)
  }
  if ("RSD[%]" %in% colnames(data_rsd)) {
    data_rsd[["RSD[%]"]] <- data_rsd[["RSD[%]"]] %>% round(1)
    if (!is.na(fill)) data_rsd[["RSD[%]"]] <- data_rsd[["RSD[%]"]] %>% fill_rsd()
  }
  # 不再修改第一列列名，因为Table S4已经有正确的列名格式
...
reportAssist <- function(inDir = "", repDir = "", repLogo = T) {
  ...
  l.a$get_table_quant_modified <<- get_table_quant_modified
