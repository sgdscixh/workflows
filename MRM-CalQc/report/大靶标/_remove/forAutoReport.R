...
get_table_calib <- function(l.a = l.a, filename = NA, sheet = "Calibration", overwrite = TRUE) {
  ...
  data_cal <- get_table_sheet(paste0(l.a$resDir, "/Calibrators.best.xlsx"))
...
reportAssist <- function(inDir = "", repDir = "", repLogo = T) {
  ...
  if (file.exists(report_temp_doc)) {
    rmarkdown::render(report_temp_doc, output_file = l.a$outWord, output_format = "officedown::rdocx_document")
  }
