...
## create word if word template exist in rmd folder
report_word <- glue::glue("{rep}/{rpt}/rmd/{tmp}", rep=rep_dir, rpt=rpt_flow, tmp="word_template.Rmd")
if (file.exists(report_word)){
  file.copy(report_word, l.a$outDir, overwrite = TRUE)
  template_word <- paste0(l.a$outDir, "/word_template.Rmd")
  out_fmt <- word_document(reference_docx = paste0(l.a$repDir, 'word_template.docx'))
  render(input = template_word, output_file = gsub('\\.pdf','.docx', outPDF), output_format = out_fmt)
  file.remove(template_word)
}
...
# --enable-local-file-access 新版wkhtmltopdf需要开启本地文件访问权限
glue::glue(
  "wkhtmltopdf --enable-local-file-access --page-size A4  --outline",
