...
python <- python <- "/worksapce/.venv/bin/python"
...
get_table_quant <- function(l.a = l.a, filename = NA, overwrite = TRUE) {
  molecule_name <- l.a$molecule_name

  data_sampleInfo <- read.xlsx(paste0(l.a$inDir, "/sampleInfo.xlsx"), na.strings = "#N/A")
  ...
  data_q1 <- get_table_sheet(paste0(l.a$resDir, "/Quantification-Raw.xlsx"))
  data_q2 <- get_table_sheet(paste0(l.a$resDir, "/Quantification-Spl.xlsx")) %>% select(-c(molecule | contains("$")))
  ...
  data_q1 <-
    data_q1 %>%
    select(-c(molecule | contains("$") | contains("HQC|LQC"))) %>%
    select(-!!qc_name_rsd)
...
get_table_calib <- function(l.a = l.a, filename = NA, sheet = "Calibration", overwrite = TRUE) {
  molecule_name <- l.a$molecule_name

  cols <- c(molecule_name, "cc_lod", "cc_loq", "xmax", "score")
  data_cal <- get_table_sheet(paste0(l.a$resDir, "/Calibrators.best.xlsx")) %>% select(c(!!cols, contains("rt_sec")))
  data_cal[, 2:3] <- round(data_cal[, 2:3], 2)
  data_cal$score <- round(data_cal$score, 4)
  name_map <- c(cc_lod = "LLOD", cc_loq = "LLOQ", xmax = "ULOQ", rt_sec = "RT(s)", score = "R^2")
  name_map[molecule_name] <- molecule_name

  colnames(data_cal) <- name_map[colnames(data_cal)]

  data_S3 <- data_cal %>% arrange(molecule_name)
  ...
  if (!is.na(filename)) {
    ...
    writeDataTable(wb, sheet = sheet, data_S3, keepNA = T, withFilter = F)
    setColWidths(wb, sheet = sheet, cols = 1:ncol(data_S3), "auto")
    setRowHeights(wb, sheet = sheet, rows = 1, 20)
    setRowHeights(wb, sheet = sheet, rows = 2:(nrow(data_S3) + 1), 16)
    addStyle(
      wb,
      sheet = sheet, rows = 1:(nrow(data_S3) + 1), cols = 1:ncol(data_S3),
...
get_table_stats <- function(l.a = l.a, filename = NA, fill = NA) {
  data_rsd <- get_table_sheet(paste0(l.a$resDir, "/Quantification-Stats.xlsx")) %>% select(-c(molecule | contains("$")))
    ...
    if (row.i == col.i) {
      ...
      p <- p + geom_blank(
        ...
        ggplot2::annotate(
          "text",
          label = round(m.cor[row.i, col.i], 3),
          x = 0.5, y = 0.5, size = 12, color = "#477aad"
        )
    } else {
      df.p <- data.frame(
        x = -log10(df.qc[, row.i]), y = -log10(df.qc[, col.i])
...
reportAssist <- function(inDir = "", repDir = "", repLogo = T) {
  ...
  # For Auto Report
  source(paste0(l.a$repDir, "oneStepGetFinalHtmlForBiotree.R"))
}
